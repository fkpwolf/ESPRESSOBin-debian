...
make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- mvebu_espressobin-88f3720_defconfig
...
# Download ARM Trusted Firmware for ESPRESSOBin if needed
if [ ! -f "${OUTPUT_DIR}/u-boot/flash-image.bin" ]; then
    ...
    make CROSS_COMPILE=aarch64-linux-gnu- PLAT=a3700 all fip
